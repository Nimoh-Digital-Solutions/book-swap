...
- [ ] A live counter or sample shows "X books available near Amsterdam" to demonstrate activity
...
**Edge Cases:**
- Visitor is outside Amsterdam → show message: "BookSwap is currently available in Amsterdam. Sign up to be notified when we launch in your area."
...
- Consider a PostGIS query to show dynamic book count for the visitor's approximate area (via IP geolocation or default to Amsterdam center)
...
**Edge Cases:**
- User enters a postcode outside Amsterdam → accept it but show: "BookSwap is currently focused on Amsterdam. You may see fewer books in your area."
...
- Snap to 500m grid: `ROUND(lat * 2) / 2, ROUND(lng * 2) / 2` (approximate — adjust precision for Amsterdam density)
- Pre-populate neighborhood dropdown with Amsterdam's main areas: Centrum, Jordaan, De Pijp, Oud-West, Oud-Zuid, Oost, Noord, West, Nieuw-West, Zuidoost, Zeeburg, Watergraafsmeer, Bos en Lommer, Westerpark, Rivierenbuurt, IJburg
...
**Edge Cases:**
- User tries a display name that's taken → inline error with suggestions (e.g., "bookworm" → "bookworm_amsterdam", "bookworm42")
...
- User at the edge of Amsterdam selects 25 km → results may include books outside Amsterdam (e.g., Amstelveen, Diemen) — this is fine
...
- Zoom out shows all of Amsterdam → clusters prevent pin overload
...
- [ ] Opens a list of curated public locations in Amsterdam, filtered by proximity to both users
...
**Technical Notes:**
- Store curated locations in a `meetup_locations` table with PostGIS points
- Start with 30–50 hand-picked locations across Amsterdam (libraries, popular cafes near book shops, central parks)
